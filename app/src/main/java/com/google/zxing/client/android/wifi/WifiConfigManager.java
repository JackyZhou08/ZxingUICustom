...
public final class WifiConfigManager extends AsyncTask<WifiParsedResult,Object,Object> {
  ...
  private static void changeNetworkWPA2EAP(WifiManager wifiManager, WifiParsedResult wifiResult) {
    ...
    config.allowedGroupCiphers.set(WifiConfiguration.GroupCipher.TKIP);
    config.allowedGroupCiphers.set(WifiConfiguration.GroupCipher.CCMP);
    config.enterpriseConfig.setIdentity(wifiResult.getDisplayResult());
    config.enterpriseConfig.setAnonymousIdentity(wifiResult.getDisplayResult());
    config.enterpriseConfig.setPassword(wifiResult.getPassword());
    config.enterpriseConfig.setEapMethod(parseEap(wifiResult.getDisplayResult()));
    config.enterpriseConfig.setPhase2Method(parsePhase2(wifiResult.getDisplayResult()));
    updateNetwork(wifiManager, config);
  }
